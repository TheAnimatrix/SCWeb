...
Deno.serve(async (req)=>{
  try {
    ...
    const { data: prRows, error: prErr } = await supabase.from('printrequests').select('id').or(`user_id.eq.${user_id},creator_id.eq.${user_id}`).eq('model', storagePath).limit(1);
    ...
    const { data: signedUrlData, error: signedUrlErr } = await supabase.storage.from('models').createSignedUrl(storagePath.replace('models/', ''), 60 * 10); // 10 min expiry
