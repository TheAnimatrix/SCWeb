...
<div class="self-center w-[50%] h-full flex flex-col mx-auto items-center max-sm:w-[95%] max-md:w-[90%] max-lg:w-[85%] max-2xl:w-[75%]">
	<div class="w-full flex flex-col items-center mb-4">
		...
		<div class="product_hero flex w-full min-h-[25%] max-sm:flex-col justify-start">
			...
			<div class="flex-[1.2] h-full flex flex-col ml-2 max-sm:ml-0">
				<div class="flex mb-2">
					<div
						class="flex-1 bg-black border-t-2 border-r-2 border-b-[12px] border-l-2 border-red-100 p-4 pl-12 flex flex-col justify-center max-sm:mt-2"
					>
				...
				<div class=" text-white pl-4 py-2 text-base text-start bg-scbgl1 mt-1 pb-4">
					<div>
					...
					<div class="flex justify-between">
						...
						<div
							id="sale_info"
							class="max-h-[55%] flex text-white text-3xl font-bold bg-gradient-to-r from-orange-500 via-orange-500 to-pink-500 text-end self-center p-2 pr-5"
						>
							Launch OFFER!
						</div>
					</div>
					<div class="flex justify-around items-center w-fit bg-scbgl2 text-xl my-2">
						<button class="p-2 px-4 hover:scale-150" on:click={dec_cart}>-</button>
						<div class="p-2 px-4 border-x-4 border-scbg" id="qty_show">{cart_qty}</div>
						<button class="p-2 px-4 hover:scale-150" on:click={inc_cart}>+</button>
						<button class="p-2 bg-black hover:scale-110 hover:rounded-md flex items-center" class:bg-scpurple="{getProduct(i).stock > 0}" id="add_to_cart" on:click={cart_submit}>
                    ...
                    <div class="flex justify-left items-stretch max-w-[70%]">
                        <div class="p-4 bg-scbgl3 text-lg">{getProduct(i).info}</div>
							{#if getProduct(i).stock > 0}
							<div class="p-4 bg-scbgl2 text-xl font-bold">
