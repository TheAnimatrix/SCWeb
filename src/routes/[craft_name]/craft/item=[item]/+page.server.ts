...
export const load: PageServerLoad = async ({ locals, params, parent }) => {
    ...
    const productResult = await locals.supabaseAdmin.from('products').select('*,users(tier,username)').eq('id', pid);
    if (productResult && !productResult.error && productResult.data && productResult.data.length > 0) {
        const product = productResult.data[0];
        const reviews = await locals.supabaseAdmin.from('reviews').select('*, users(username,tier)').eq('product_id', pid).order('created_at', { ascending: false });
